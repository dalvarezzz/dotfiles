...
keys.client_keys = gears.table.join(
	keys.client_keys,
	awful.key({ user.modkey }, 'f', function(c)
		c.fullscreen = not c.fullscreen
		c:raise()
	end, { description = 'toggle fullscreen', group = 'client' }),
	awful.key({ user.modkey, 'Shift' }, 'c', function(c) c:kill() end, { description = 'close', group = 'client' }),
	awful.key(
		{ user.modkey, 'Control' },
		'space',
	...
	awful.key({ user.modkey }, 'n', function(c)
		-- The client currently has the input focus, so it cannot be
		-- minimized, since minimized clients can't have the focus.
		c.minimized = true
	end, { description = 'minimize', group = 'client' }),
	awful.key({ user.modkey }, 'm', function(c)
		c.maximized = not c.maximized
		c:raise()
	end, { description = '(un)maximize', group = 'client' }),
	awful.key({ user.modkey, 'Control' }, 'm', function(c)
		c.maximized_vertical = not c.maximized_vertical
		c:raise()
	end, { description = '(un)maximize vertically', group = 'client' }),
	awful.key({ user.modkey, 'Shift' }, 'm', function(c)
...
keys.global_keys = gears.table.join(
	keys.global_keys,
	awful.key({ user.modkey }, 'Left', awful.tag.viewprev, { description = 'view previous', group = 'tag' }),
	awful.key({ user.modkey }, 'Right', awful.tag.viewnext, { description = 'view next', group = 'tag' }),
	awful.key({ user.modkey }, 'Escape', awful.tag.history.restore, { description = 'go back', group = 'tag' }),
	awful.key(
		{ user.modkey },
		'j',
	...
	awful.key({ user.modkey }, 'Tab', function()
	...
	awful.key({ user.modkey, 'Control' }, 'r', awesome.restart, { description = 'reload awesome', group = 'awesome' }),
	awful.key({ user.modkey, 'Shift' }, 'q', awesome.quit, { description = 'quit awesome', group = 'awesome' }),
	awful.key(
		{ user.modkey },
		'l',
	...
	awful.key({ user.modkey, 'Control' }, 'n', function()
...
for i = 1, 9 do
	keys.global_keys = gears.table.join(
		keys.global_keys,
		-- View tag only.
		awful.key({ user.modkey }, '#' .. i + 9, function()
		...
		-- Toggle tag display.
		awful.key({ user.modkey, 'Control' }, '#' .. i + 9, function()
			local screen = awful.screen.focused()
			local tag = screen.tags[i]
			if tag then
		...
		-- Move client to tag.
		awful.key({ user.modkey, 'Shift' }, '#' .. i + 9, function()
			if client.focus then
		...
		awful.key({ user.modkey, 'Control', 'Shift' }, '#' .. i + 9, function()
			if client.focus then
				local tag = client.focus.screen.tags[i]
				if tag then
